<main class="min-h-screen bg-[#f9fafc] pt-[74px]">
	<section class="mx-auto grid w-full max-w-screen-xl gap-5 p-5">
		<div class="flex flex-col items-start justify-between gap-y-2 rounded-lg border border-slate-100 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
			...
			<div>
				<x-select name="year" :multiple="false" item-select-text="" :search="false" class="min-w-20" :options="[
				    (object) [
				        'label' => '2022',
				        'value' => '2022',
				    ],
				...
				<div class="space-y-5 p-6">
					@foreach ($indikator->butir as $butir)
						<div class="space-y-3">
							...
							<x-select name="{{ $butir->kode }}" :multiple="false" :search="false" class="w-52" placeholder="Ketersediaan Dokumen" :options="[
							    (object) [
							        'label' => 'Tersedia',
							        'value' => 'Tersedia',
							    ],
							...
							<textarea rows="4" class="block w-full rounded-md border border-neutral-200 bg-neutral-100 p-4 text-sm text-slate-600 focus:outline-none focus:outline-color-info-500" placeholder="Catatan..."></textarea>
